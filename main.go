...
func main() {
	...
	a := app.New()
	w := a.NewWindow("New Useless Fact")
	...
	factTxt := widget.NewLabel("")
	factTxt.Wrapping = fyne.TextWrapWord
	...
	button := widget.NewButton("Get Fact", func() {
		...
		if err != nil {
			...
			factTxt.SetText(fact.Text)
		}
	})

	hBox := container.New(layout.NewHBoxLayout(), layout.NewSpacer(), button, layout.NewSpacer())
	vBox := container.New(layout.NewVBoxLayout(), title, hBox, widget.NewSeparator(), factTxt)
